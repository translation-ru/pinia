...
### Выборочное создание заглушек для действий %{#selective-action-stubbing}%

Иногда вам может потребоваться создать заглушки только для определённых действий, позволяя другим выполняться нормально. Вы можете добиться этого, передав массив имён действий в опцию `stubActions`:

```js
// Создать заглушки только для действий 'increment' и 'reset'
const wrapper = mount(Counter, {
  global: {
    plugins: [
      createTestingPinia({
...
// Эти действия будут заменены заглушками (не будут выполняться)
store.increment() // заглушка
store.reset() // заглушка

// Другие действия будут выполняться нормально, но за ними всё равно будет вестись наблюдение
store.fetchData() // выполняется нормально
expect(store.fetchData).toHaveBeenCalledTimes(1)
```

Для более сложных сценариев вы можете передать функцию, которая получает имя действия и экземпляр хранилища и возвращает, должно ли действие быть заменено заглушкой:

```js
// Создание заглушек на основе пользовательской логики
const wrapper = mount(Counter, {
  global: {
    plugins: [
      createTestingPinia({
        stubActions: (actionName, store) => {
          // Создать заглушки для всех действий, начинающихся с 'set'
          if (actionName.startsWith('set')) return true

          // Создать заглушки на основе начального состояния хранилища
...
// Действия, начинающиеся с 'set', заменены заглушками
store.setValue(42) // заглушка

// Другие действия могут выполняться на основе начального состояния хранилища
store.fetchData() // выполняется или заменяется заглушкой на основе начального store.isPremium
```

::: tip Совет

- Пустой массив `[]` означает, что никакие действия не будут заменены заглушками (то же самое, что `false`)
- Функция вычисляется один раз при создании хранилища и получает экземпляр хранилища в его начальном состоянии
:::

Вы также можете вручную мокировать конкретные действия после создания хранилища:

```js
const store = useSomeStore()
vi.spyOn(store, 'increment').mockImplementation(() => {})
// или если в тестовой Pinia действия заменены заглушками
